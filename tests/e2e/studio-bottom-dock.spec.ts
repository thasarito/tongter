...
test("More exposes files, layers and sheet reload without returning a top toolbar",async({page})=>{
  ...
  const review=page.locator('[data-sheet-confirmation]');await expect(review).toHaveText('Reload Google SheetsCancelConfirm');expect(sheet.writes).toHaveLength(0);await review.getByRole('button',{name:'Confirm',exact:true}).click();await expect(review).toBeHidden();
...
test("real guest dragging fades mobile tools and can target a seat beneath the sheet",async({page,isMobile},testInfo)=>{
  ...
  const review=page.locator('[data-sheet-confirmation]');await expect(review).toContainText('Waiting 12');await expect(review).toContainText('Table 1 · Seat 8');expect(sheet.writes).toHaveLength(0);await review.getByRole('button',{name:'Confirm',exact:true}).click();
