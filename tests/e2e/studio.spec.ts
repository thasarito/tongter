...
test("saves one swap on drop with bounded feedback and no JSON controls",async({page,isMobile},testInfo)=>{
  ...
  await expect(page.getByRole("button",{name:"Save JSON",exact:true})).toHaveCount(0);await expect(page.locator('.studio-toolbar input[type=file]')).toHaveAttribute("accept",".csv,text/csv");
