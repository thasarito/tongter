...
describe("durable receipt-based sheet writer",()=>{
 ...
 it("reconciles a lost response from the atomic receipt",async()=>{const f=fixture(),write=f.gateway.write;f.gateway.write=vi.fn(async(...args:Parameters<StudioGateway["write"]>)=>{await write(...args);throw new SheetWriteError(false);});expect((await f.engine.execute(f.operation)).status).toBe(200);expect(f.getPending()).toBeUndefined();});
