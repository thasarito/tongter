...
export function PlanSeat({table,number,layout,onSelect}:{table:StudioItem;number:number;layout:StudioLayout;onSelect?:()=>void}){
  ...
  return <g transform={`translate(${seat.local.join(" ")})`} {...drop} data-guest-drag={g?.id} className="studio-seat" role="button" tabIndex={0} aria-label={`Table ${table.label}, seat ${number}: ${g?.name??"Empty"}`} onClick={e=>{e.stopPropagation();open();}} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();e.stopPropagation();open();}}>
