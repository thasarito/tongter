...
export default function FloorPlan(){
  ...
  const displayed=useMemo(()=>{if(!preview)return layout;return {...layout,items:layout.items.map(t=>t.id===preview.id?{...t,x:preview.x,z:preview.z}:t)};},[layout,preview]);
  ...
  useViewportActions({"zoom-in":()=>camera.zoom(.8),"zoom-out":()=>camera.zoom(1.25),fit:()=>camera.fit(labels),svg:()=>{if(camera.svg.current)exportPlanSvg(camera.svg.current);},png:()=>{if(camera.svg.current)void exportPlanPng(camera.svg.current).catch(e=>notify(e instanceof Error?e.message:"PNG export failed"));}});
  return <div className="studio-plan" aria-label="Editable floor plan">
    ...
    </svg><p className="studio-canvas-hint">Drag empty space to pan · pinch to zoom · drag furniture to move · {warnings.length} spatial warnings (not an egress approval)</p>
  </div>;
}
