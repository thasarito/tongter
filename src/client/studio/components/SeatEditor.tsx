...
function SeatRow({target,selected,onSelect}:{target:SeatTarget;selected:boolean;onSelect:()=>void}){
  ...
  return <div className={`studio-seat-row ${selected?"selected":""} ${g?"occupied":""}`} {...drop}>{g&&<GuestGrip id={g.id}/>}<button onClick={onSelect} aria-pressed={selected}><b>{target.seatNumber}</b><span>{g?.name??"Empty — assign guest"}<small>{g?[g.host,g.group].filter(Boolean).join(" · "):"No guest assigned"}</small></span></button></div>;
}
export function SeatEditor({target}:{target:SeatTarget}) {
  ...
  const effective=table?Math.max(1,Math.min(number,table.seats)):1,seatTarget={tableId:target.tableId,seatNumber:effective},current=occupant(layout,seatTarget),chosen=layout.guestList.find(g=>g.id===candidate);
  const labels=useMemo(()=>table?diagramLabels(layout,table):[],[layout,table]);
  ...
  const minY=Math.min(-table.w/2-1,-table.d/2-1,...labels.map(l=>l.y-.2)),maxY=Math.max(table.w/2+1,table.d/2+1,...labels.map(l=>l.y+l.h+.2));
  ...
  function apply(){if(!chosen)return;try{
  ...
  return <Modal title={`Around Table ${table.label}`} wide onClose={()=>setModal(null)}>
    <div className="studio-row"><label>Table<select value={table.id} onChange={e=>setModal({type:"seat",target:{tableId:e.target.value,seatNumber:1}})}>{layout.items.filter(t=>t.kind==="table").map(t=><option key={t.id} value={t.id}>Table {t.label} · {tableGuests(layout,t.id).length}/{t.seats}</option>)}</select></label><button disabled={!canUndo} onClick={undo}>↶ Undo</button><button disabled={!canRedo} onClick={redo}>↷ Redo</button><button onClick={()=>exportSeatingHtml(layout,table.id)}>Export this table</button></div>
    <div className="studio-seat-editor"><section><p className="studio-note">Green = assigned · cream = empty. Click a chair/name or drag between seats. Seat 1 starts on the table's local right; numbering rotates with it.</p>
      <svg className="studio-seat-diagram" viewBox={`-4.2 ${minY} 8.4 ${maxY-minY}`} aria-label={`Table ${table.label} seat map`}>
        {options.guestNames&&labels.map(label=><GuestNameBadge key={label.key} label={label} onSelect={()=>select(label.seatNumber)}/>)}
        <g transform={`rotate(${table.rotation})`}><ellipse rx={table.w/2} ry={table.d/2} fill="#e5eadb" stroke="#aebca1" strokeWidth={.025}/>{seats(table).map(p=><PlanSeat key={p.number} table={table} number={p.number} layout={layout} onSelect={()=>select(p.number)}/>)}</g>
        <text y={.1} textAnchor="middle" fontSize={.35} fill="#657b59">{table.label}</text>
      </svg>
      <h3>Who sits where</h3><div className="studio-seat-list">{seats(table).map(p=><SeatRow key={p.number} target={{tableId:table.id,seatNumber:p.number}} selected={p.number===effective} onSelect={()=>select(p.number)}/>)}</div>
    </section><section className="studio-seat-assignment"><h3>Seat {effective}</h3><div className="studio-current-guest"><strong>{current?.name??"Ready for a guest"}</strong><small>{current?[current.host,current.group].filter(Boolean).join(" · "):"Empty seat"}</small>{current&&<div className="studio-row"><button onClick={()=>setModal({type:"guest",id:current.id,target:seatTarget})}>Edit guest</button><button onClick={()=>commit("Guest unassigned; record retained",s=>moveGuests(s,[current.id],{tableId:""}))}>Clear seat</button></div>}</div>
      ...
      <input type="search" aria-label="Search guests for seat" placeholder="Search a name, side or group" value={query} onChange={e=>setQuery(e.target.value)}/><label><input type="checkbox" checked={all} onChange={e=>setAll(e.target.checked)}/> Include seated guests for moving / swapping</label>
      ...
      <button className="primary" disabled={!chosen||chosen.reserve||chosen.rsvp==="Declined"} onClick={apply}>{current?(chosen?.tableId?"Swap seats":"Replace occupant…"):`Assign to seat ${effective}`}</button><button disabled={!!current} onClick={()=>setModal({type:"guest",target:seatTarget})}>+ Add a new guest here</button>
    </section></div>
  </Modal>;
}
