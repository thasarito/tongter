...
export function GuestForm({id,target}:{id?:string;target?:SeatTarget}) {
  ...
  return <Modal title={old?"Edit guest":"Add guest"} onClose={()=>setModal(target?{type:"seat",target}:null)}><form onSubmit={submit} className="studio-form">
    ...
    {error&&<p role="alert">{error}</p>}<footer>{old&&<button type="button" className="danger" onClick={()=>{if(confirm(`Remove ${old.name}? Their seat will be released. Undo is available.`)){commit("Guest removed",s=>({...s,guestList:s.guestList.filter(g=>g.id!==old.id)}));setModal(null);}}}>Remove guest</button>}<button className="primary" type="submit">Save guest</button></footer>
  </form></Modal>;
}
