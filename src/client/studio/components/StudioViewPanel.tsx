...
export function StudioViewPanel(){
  const {view,setView,notify,layout}=useStudio(),{viewport,closePanel}=useStudioChrome();
  ...
  return <section className="studio-view-panel">
    ...
    {view==="model"&&<><button disabled={!layout.items.some(item=>item.kind==="stage")} onClick={()=>{if(viewport.run("pdf-reference"))closePanel();else notify("The view is still loading. Try again when it appears.");}}>Match PDF framing</button><p className="studio-note">Front-on stage view for PDF pages 3–5. Changing Stage setup adjusts the framing. Fit room restores the overview.</p></>}
