...
export function ImportDialog({data}:{data:GuestImport}){
  ...
  return <Modal title="Review guest CSV import" onClose={()=>setModal(null)}>
    ...
    {preview.error?<p role="alert">{preview.error}</p>:<p>{preview.result?.layout.guestList.length} guests after import. Applying will save the reviewed changes to StudioGuests.</p>}
    ...
    <footer><button onClick={()=>setModal(null)}>Cancel</button><button className="primary" disabled={!preview.result||!editable} onClick={()=>{if(!confirm(mode==="replace"?"Replace the live StudioGuests roster with this CSV? Removed guest rows will be deleted. Furniture and invitation/RSVP tabs stay unchanged.":"Save this reviewed CSV merge to Google Sheets?"))return;commit("Saving guest CSV changes",current=>applyGuestImport(current,data,mode).layout);setModal(null);}}>Apply reviewed import</button></footer>
  </Modal>;
}
