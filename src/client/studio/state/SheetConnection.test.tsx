...
describe("manual sheet connection reviews",()=>{
 it("confirms read-only reload but never prompts for a focus refresh",async()=>{
  ...
  expect(screen.getByRole("dialog")).toHaveTextContent("read-only");expect(reads).toBe(1);
  approve("Confirm reload");await waitFor(()=>expect(reads).toBe(2));
