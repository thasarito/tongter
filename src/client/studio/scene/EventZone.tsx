...
export function EventZone({item}:{item:StudioItem}){
  ...
  return <group position={[item.x,.012,item.z]} rotation={[0,-radians(item.rotation),0]}>
    {decorated&&item.shape==="rect"?stageTiers(item).map((tier,i)=><mesh key={i} position={[-tier.front+tier.depth/2,tier.height/2,0]} castShadow receiveShadow={false} onClick={e=>{e.stopPropagation();if(view!=="inside")setSelected(item.id);}}>
