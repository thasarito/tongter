...
export function WalkController({motion}:{motion:WalkMotion}){
  ...
  useEffect(()=>{
    ...
    const down=(e:KeyboardEvent)=>{if(editing()||!motion.enabled||document.querySelector("dialog[open]")||e.metaKey||e.ctrlKey||e.altKey)return;const direction=directions[e.code];if(direction){e.preventDefault();motion.press(`key:${e.code}`,direction);}motion.fast=e.shiftKey;if(e.key==="Escape")stop();};
    ...
    const moveLook=(e:PointerEvent)=>{if(!pointer||pointer.id!==e.pointerId)return;e.preventDefault();motion.look(e.clientX-pointer.x,e.clientY-pointer.y);pointer.x=e.clientX;pointer.y=e.clientY;};
