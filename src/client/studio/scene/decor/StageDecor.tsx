...
export function DecorBox({position,size,color=DECOR_COLORS.fabric,receiveShadow=true}:{position:Vec3;size:Vec3;color?:string;receiveShadow?:boolean}){
  return <mesh position={position} raycast={noDecorRaycast} castShadow receiveShadow={receiveShadow}><boxGeometry args={size}/><meshStandardMaterial color={color} roughness={.92}/></mesh>;
}
...
/** The repository's path-only wedding wordmark, rendered as actual SVG outlines. */
export function WeddingWordmark({position,width=.78}:{position:Vec3;width?:number}){
  const data=useLoader(SVGLoader,"/logo.svg");
  const geometry=useMemo(()=>wordmarkGeometry(data),[data]);
  useEffect(()=>()=>geometry.dispose(),[geometry]);
  return <mesh name="decor-wedding-wordmark" position={position} scale={[width,width,1]} geometry={geometry} raycast={noDecorRaycast}>
    <meshBasicMaterial color="#231f20" side={DoubleSide} toneMapped={false}/>
  </mesh>;
}
const stageFlowers:FlowerClump[]=[
  ...
  {center:[1.25,1.02,-.11],radius:[.18,.45,.17],count:160,seed:85},
...
export function DrapedTable({width=2.2,depth=.72,height=.78}:{width?:number;depth?:number;height?:number}){
  return <group name="decor-draped-table">
    ...
    <Fabric position={[0,0,depth/2+.01]} width={width+.08} height={height} gather={1} pool={.10} opacity={1}/>
    <Fabric position={[0,0,-depth/2-.01]} rotation={[0,Math.PI,0]} width={width+.08} height={height} gather={1} pool={.08} opacity={1}/>
    {[-1,1].map(sign=><Fabric key={sign} position={[sign*width/2,0,0]} rotation={[0,sign*Math.PI/2,0]} width={depth+.05} height={height} gather={1} pool={.09} opacity={1}/>)}
...
function CakeTable(){
  return <group name="decor-cake-table" position={[0,0,.12]}>
...
export function StageDecor({stage,mode}:{stage:StudioItem;mode:DecorationMode}){
  return <group name="decor-stage-assembly" {...decorTransform(stage)}>
    <DecorBox position={[0,1.20,-1.10]} size={[5.08,2.4,.055]} color={DECOR_COLORS.board} receiveShadow={false}/>
    ...
    {[0,.12,.25,.38].map((drop,i)=><Fabric key={i} kind="swag" position={[0,0,-.86+i*.03]} width={3.15+i*.09} height={3.58-drop} sag={2.56} band={.25} opacity={.87}/>)}
    ...
    {[-1.85,-.59,.76,2].map((x,i)=><Fabric key={i} kind="swag" position={[x,0,.10]} width={1.8} height={5.12-i%2*.2} sag={1.07+i%2*.2} band={.30} opacity={.76}/>)}
