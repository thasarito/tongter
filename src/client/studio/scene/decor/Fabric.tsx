...
interface FabricProps {
  ...
  pool?: number; tilt?: number; opacity?: number;
}
/** Pleated, double-sided cloth surfaces, rather than flat triangular stand-ins. */
export function Fabric({kind="curtain",position,rotation,width,height,sag=1,band=.24,lean=0,pool=.4,tilt=0,opacity=.94}:FabricProps){
  const geometry=useMemo(()=>{
    const nu=64,nv=32,positions:number[]=[],indices:number[]=[];
    for(let j=0;j<=nv;j++)for(let i=0;i<=nu;i++){
      const u=i/nu,v=j/nv;
      positions.push(...(kind==="swag"?swagPoint(u,v,{width,height,sag,band,tilt}):kind==="round"?roundClothPoint(u,v,width/2,height):curtainPoint(u,v,{width,height,lean,pool})));
    }
  ...
  },[kind,width,height,sag,band,lean,pool,tilt]);
