...
interface Session {pointerId:number;handle:Element;ids:string[];x:number;y:number;startX:number;startY:number;active:boolean;focus:boolean;revision:number}
...
export function GuestDragProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{
    ...
    function down(e:PointerEvent){
      ...
      session.current={pointerId:e.pointerId,handle:el,ids,x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY,active:false,focus:false,revision:state.revision};
    ...
    function move(e:PointerEvent){
      ...
      if(!s.active){
        s.active=true;
        s.focus=!document.querySelector("dialog[open]")&&(innerWidth<=760||(innerHeight<=520&&matchMedia("(pointer: coarse)").matches));
      ...
      // Desktop and modal lists retain edge scrolling. A faded mobile panel must
      // keep its previous scroll position and cannot become the destination.
    ...
    function up(e:PointerEvent){
      ...
      // incorrectly catch a drop intended for a seat beneath the guest sheet.
