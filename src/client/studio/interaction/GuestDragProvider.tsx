...
export function GuestDragProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{
    ...
    function clear(cancel=false){
      const s=session.current;session.current=null;document.body.classList.remove("studio-pointer-dragging");if(!s)return;
      try{if(s.handle.hasPointerCapture(s.pointerId))s.handle.releasePointerCapture(s.pointerId);}catch{/* source may have unmounted */}
      setFeedback(idle);if(s.active){suppress.current=performance.now()+400;if(cancel)latest.current.notify("Move cancelled. Assignments unchanged.");}
    }
    ...
    function move(e:PointerEvent){
      ...
      s.active=true;e.preventDefault();e.stopPropagation();document.body.classList.add("studio-pointer-dragging");
      ...
      let el=document.elementFromPoint(s.x,s.y);while(el&&el!==document.body){const style=getComputedStyle(el);if(/auto|scroll/.test(style.overflowY)&&el.scrollHeight>el.clientHeight){const r=el.getBoundingClientRect();if(s.y<r.top+30)el.scrollTop-=12;else if(s.y>r.bottom-30)el.scrollTop+=12;break;}el=el.parentElement;}
    }
    ...
    document.addEventListener("pointerdown",down,{capture:true,passive:false});document.addEventListener("pointermove",move,{capture:true,passive:false});document.addEventListener("pointerup",up,true);document.addEventListener("pointercancel",pointerCancel,true);document.addEventListener("lostpointercapture",pointerCancel,true);document.addEventListener("click",click,true);document.addEventListener("keydown",key,true);document.addEventListener("dragstart",nativeDrag,true);document.addEventListener("visibilitychange",visibility);window.addEventListener("blur",cancel);
    return()=>{session.current=null;document.body.classList.remove("studio-pointer-dragging");document.removeEventListener("pointerdown",down,true);document.removeEventListener("pointermove",move,true);document.removeEventListener("pointerup",up,true);document.removeEventListener("pointercancel",pointerCancel,true);document.removeEventListener("lostpointercapture",pointerCancel,true);document.removeEventListener("click",click,true);document.removeEventListener("keydown",key,true);document.removeEventListener("dragstart",nativeDrag,true);document.removeEventListener("visibilitychange",visibility);window.removeEventListener("blur",cancel);};
  },[]);
